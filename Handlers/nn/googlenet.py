...
class GoogleNet:
    ...
    def build(width,height,depth,classes,reg=0.005):
        ...
        x=GoogleNet.incention_module(x,64,96,128,16,32,32,chanDim,'3a',reg=reg)
        x=GoogleNet.incention_module(x,128,128,192,32,96,64,chanDim,'3b',reg=reg)
        x=MaxPooling2D((3,3),strides=(2,2),padding='same',name='pool3')(x)
        x=GoogleNet.incention_module(x,192,96,208,16,48,64,chanDim,'4a',reg=reg)
        x=GoogleNet.incention_module(x,160,112,224,24,64,64,chanDim,'4b',reg=reg)
        x=GoogleNet.incention_module(x,128,128,256,24,64,64,chanDim,'4c',reg=reg)
        x=GoogleNet.incention_module(x,112,144,288,32,64,64,chanDim,'4d',reg=reg)
        x=GoogleNet.incention_module(x,256,160,320,32,128,128,chanDim,'4e',reg=reg)
